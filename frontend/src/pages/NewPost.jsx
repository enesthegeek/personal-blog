...
export default function NewPost() {
  ...
  async function handleSubmit(e) {
    ...
    setError('');
    try {
      const post = await createPost({ title: title.trim(), content, category, tags, coverImage: coverUrl, author: 'Enes' });
      navigate(`/post/${post.id}`);
    } catch {
      setError('Failed to publish. Make sure the backend is running.');
      setSubmitting(false);
    }
  }
    ...
    <div style={{ paddingTop: 80 }}>
      <div className="wrap" style={{ paddingTop: 48, paddingBottom: 100 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          <Field label="Cover Image" mono>
            <div
              ...
              style={{
                ...
                borderRadius: 'var(--radius)',
                background: coverPreview ? 'transparent' : 'var(--bg-card)',
                cursor: coverLoading ? 'wait' : 'pointer',
                overflow: 'hidden',
                transition: 'border-color 0.2s',
                minHeight: coverPreview ? 0 : 120,
                display: 'flex',
                alignItems: 'center',
            ...
            >
              {coverPreview ? (
                <>
                  <img src={coverPreview} alt="Cover preview" style={{ width: '100%', maxHeight: 280, objectFit: 'cover', display: 'block' }} />
                  <button
                    type="button"
                    onClick={e => { e.stopPropagation(); setCoverPreview(null); setCoverUrl(''); }}
                    style={{
                ...
                {tags.map(tag => (
                  <span key={tag} style={{
                    ...
                    <button type="button" onClick={() => setTags(tags.filter(t => t !== tag))}
                      style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', padding: 0, lineHeight: 1, fontSize: 12 }}>
                      ✕
                    </button>
                  </span>
                ))}
          ...
          {/* Error */}
          {error && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#f87171' }}>{error}</p>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button
              ...
              style={{
                padding: '12px 32px',
                background: `linear-gradient(135deg, ${accent} 0%, ${accentBorder} 100%)`,
                border: 'none',
                borderRadius: 'var(--radius)',
                color: category === 'nuclear' ? '#0a0a0a' : '#0a0a0a',
                fontFamily: 'var(--font-sans)',
                fontSize: 14,
                fontWeight: 600,
              ...
              onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = `0 0 0 0 ${accentGlow}`; }}
            >
